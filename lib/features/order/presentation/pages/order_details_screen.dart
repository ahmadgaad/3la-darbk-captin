import 'package:ala_darbak_captain/temp/app_temp.dart';
              ...
              : LoadingOverlay(
                ...
                child: Scaffold(
                    ...
                    body: RefreshIndicator(
                      ...
                      child: ListView(
                        padding:
                            EdgeInsets.symmetric(horizontal: 16.w, vertical: 20),
                        cacheExtent: 20,
                        children: [
                        ...
                        _textBuilder(AppStrings.tripNumber,
                            '#${state.orderModel?.trip?.numTrip ?? ""}'),
                        10.verticalSpaceFromWidth,
                        _textBuilder(AppStrings.startCity,
                            state.orderModel?.trip?.cityFrom?.name ?? ""),
                        10.verticalSpaceFromWidth,
                        _textBuilder(AppStrings.destenationCity,
                            state.orderModel?.trip?.cityTo?.name ?? ""),
                        10.verticalSpaceFromWidth,
                        const Divider(),
                      },
                          10.verticalSpace,
                          _textBuilder(AppStrings.orderCategory,
                      ...
                      _textBuilder(
                        ...
                        '${state.orderModel?.quantity ?? 1}',
                          ...
                          const Divider(),
                          _textBuilder(AppStrings.orderPrice,
                            ...
                            children: [
                              _textBuilder(
                                  ...
                                  state.orderModel?.paymentMethod == "0"
                                      ? AppStrings.cash
                                      : AppStrings.online),
                              Text(isPaid ? AppStrings.paid : AppStrings.notPaid,
                          ...
                          10.verticalSpace,
                          OrderImages(
                            images: state.orderModel?.images ?? [],
                          ),
                          10.verticalSpace,},
                          const Divider(),
                          10.verticalSpace,
                          _textBuilder(AppStrings.addtionalDetails,
                              state.orderModel?.note ?? ""),
                      if(!isPerson)...{    10.verticalSpace,
                          const Divider(),
                          10.verticalSpace,
                    ...
                    bottomNavigationBar: status > 2
                        ...
                        : Padding(
                            ...
                            child: Column(
                              ...
                              spacing: 15.h,
                              children: [
                                if (status == 2 && !isPaid&&   state.orderModel?.paymentMethod == "0")
                                  ElevatedButton(
                                      style: ElevatedButton.styleFrom(
                                          backgroundColor: AppColors.success),
                                      onPressed: cubit.payOrder,
                                      child: const Text(AppStrings.pay)),
                                ElevatedButton(
                                    style: ElevatedButton.styleFrom(
                                ...
                                if (status == 1||(status==0 && state.orderModel?.tripId!=null))
                                  ElevatedButton(
                                      onPressed: () {
                                        cubit.updateOrder(4);
                                      },
                                      child: const Text(AppStrings.disApproved)),
                              ],
                            ),
                          ),
                  ),
        ...
        children: [
          ...
          Flexible(
            child: Column(
              ...
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(state.orderModel?.client?.name ?? '',
                    style: AppTextStyle.font18black600),
                InkWell(
                  onTap: () async {
                    final url =
                        "tel:+966${state.orderModel?.client?.mobile ?? ''}";
                    if (await launchUrl(Uri.parse(url))) {
                      print('can\'t launch $url');
                    }
                  },
                  child: Text("+966${state.orderModel?.client?.mobile ?? ''}",
